...
KINLINE i32 findMemoryIndex(u32 typeFilter, u32 propertyFlags);

global_variable vulkanContext context;

global_variable u32 cachedFramebufferWidth = 0;

global_variable u32 cachedFramebufferHeight = 0;

KINLINE b8 create_buffers(vulkanContext * context){
...
KINLINE void upload_data_range(vulkanContext * context, VkCommandPool pool, VkFence fence, VkQueue queue, vulkan_buffer * buffer,   u64 offset,
u64 size, void * data){
...
KINLINE void create_command_buffers(renderer_backend * backend){
...
KINLINE void regenerate_frame_buffers(renderer_backend* backend, vulkanSwapchain * swapchain, 
...
KINLINE void platform_get_required_extension_names(const char ***names_darray) {

    darray_push(*names_darray, &"VK_KHR_xcb_surface");  // VK_KHR_xlib_surface?

}

KINLINE b8 recreateSwapchain(renderer_backend * backend){
    ...
    regenerate_frame_buffers(backend, &context.swapchain, &context.mainRenderPass);
...
KINLINE b8 vulkanRendererBackendInitialize(renderer_backend* backend, const char* applicationName){
...
KINLINE void vulkan_renderer_update_global_state(mat4 projection, mat4 view, vec3 view_position, vec4 ambient_colour, i32 mode){
