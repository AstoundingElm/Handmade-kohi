...
if(xcb_connection_has_error(state->connection)){
    KFATAL("Failed to connect to xorg server via xcb");
    return false;
...
state->screen->root, 
x, y, width, height, 0, XCB_WINDOW_CLASS_INPUT_OUTPUT, state->screen->root_visual, eventMask, valueList);
...
static inline void platformShutdown(platformState * platState){
internalState* state = (internalState*)platState->internalState;
...
keys translate_key_code(u32 x_keycode) {
    switch (x_keycode) {
        case XK_BackSpace:
...
static inline b8 platformPumpMessages(platformState * platState){
internalState* state = (internalState*)platState->internalState;
 ...
 while (event != 0) {
        ...
        switch (event->response_type & ~0x80) {
            ...
            case XCB_KEY_RELEASE: {
                ...
                keys key = translate_key_code(keySym);
...
static inline void * platformAllocate(u64 size, b8 aligned){

    return malloc(size);
};

static inline void platformFree(void * block, b8 aligned){
    
    free(block);

}

static inline void * platformZeroMemory(void * block, u64 size){
    
    return memset(block, 0, size);
}

static inline void * platformCopyMemory(void * dest, const void * src, u64 size){
    
    return memcpy(dest, src, size);
}

static inline void * platformSetMemory(void * dest, i32 value, u64 size){
    return memset(dest, value, size);
}

static inline f64 platformGetAbsoluteTime(){
...
static inline void platformSleep(u64 ms){
    #if _POSIX_C_SOURCE >= 199309L
     timespec ts;
    ts.tv_sec = ms / 1000;
