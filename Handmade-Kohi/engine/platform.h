...
PFN_vkCreateInstance MydefvkCreateInstance;



       MydefvkCreateInstance = (PFN_vkCreateInstance) vkGetInstanceProcAddr(NULL, "vkCreateInstance");
...
static KAPI b8 eventFire(u16 code, void * sender, eventContext context);
...
KINLINE b8 platformStartup(platformState * plat_state, const char * applicationName, i32 x, i32 y, i32 width, i32 height){
...
KINLINE void platformShutdown(platformState * plat_state){
internal_state* state = (internal_state*)plat_state->internal_state;
...
static keys translate_key_code(u32 x_keycode) {
    switch (x_keycode) {
        case XK_BackSpace:
...
static  void inputProcessKey(keys key, b8 pressed);
static void  inputProcessButton(buttons button, b8 pressed);
static void inputProcessMouseMove(i16 x, i16 y);
KINLINE b8 platformPumpMessages(platformState * plat_state){
 internal_state* state = (internal_state*)plat_state->internal_state;
    ...
    while (event != 0) {
        ...
        switch (event->response_type & ~0x80) {
            ...
            case XCB_KEY_RELEASE: {
                ...
                keys key = translate_key_code(key_sym);
...
KINLINE void * platformAllocate(u64 size, b8 aligned){

    return malloc(size);
};

KINLINE void platformFree(void * block, b8 aligned){
    
    free(block);

}

KINLINE void * platformZeroMemory(void * block, u64 size){
    
    return memset(block, 0, size);
}

KINLINE void * platformCopyMemory(void * dest, const void * src, u64 size){
    
    return memcpy(dest, src, size);
}

KINLINE void * platformSetMemory(void * dest, i32 value, u64 size){
    return memset(dest, value, size);
}

KINLINE f64 platformGetAbsoluteTime(){
...
KINLINE void platformSleep(u64 ms){
    #if _POSIX_C_SOURCE >= 199309L
     timespec ts;
    ts.tv_sec = ms / 1000;
