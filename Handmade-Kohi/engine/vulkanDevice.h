...
if (!selectPhysicalDevice(context)) {
    ...
    VK_CHECK(vkCreateCommandPool(context->device.logicalDevice, &poolCreateInfo, context->allocator, 
 ...
 void vulkanDeviceDestroy(vulkanContext * context){
  context->device.graphicsQueue = 0;
    ...
    context->device.graphicsCommandPool, context->allocator);
...
static inline b8 vulkanDeviceDetectDepthFormat(vulkanDevice * device){
