...
KINLINE b8 vulkan_graphics_pipeline_create(vulkanContext * context, vulkanRenderpass * renderpass, u32 attribute_count, 
VkVertexInputAttributeDescription * attributes, u32 descriptor_set_layout_count, VkDescriptorSetLayout * descriptor_set_layouts,
u32 stage_count, VkPipelineShaderStageCreateInfo * stages, VkViewport viewport, VkRect2D scissor, b8 is_wireframe,
vulkan_pipeline * out_pipeline){
    ...
    kzeroMemory(&color_blend_attachment_state, sizeof(VkPipelineColorBlendAttachmentState));
    ...
    KERROR("vkCreateGraphicsPipelines failed with %s.", vulkanResultString(result, true));
    return false;


};

KINLINE void vulkan_pipeline_destroy(vulkanContext * context, vulkan_pipeline * pipeline){
     ...
     if (pipeline) {
        ...
        if (pipeline->pipeline_layout) {
            vkDestroyPipelineLayout(context->device.logicalDevice, pipeline->pipeline_layout, context->allocator);
...
KINLINE void vulkan_pipeline_bind(vulkanCommandBuffer * command_buffer, VkPipelineBindPoint bind_point, vulkan_pipeline * pipeline){
