...
KINLINE KAPI b8 filesystem_exists(const char * path){
...
KINLINE KAPI b8 filesystem_open(const char * path, file_modes mode, b8 binary, file_handle * out_handle){
...
KINLINE KAPI b8 filesystem_close(file_handle * handle){
...
KINLINE KAPI b8 file_system_read_line(file_handle * handle, char ** line_buf){
      ...
      if (handle->handle) {
        ...
        if (fgets(buffer, 32000, (FILE*)handle->handle) != 0) {
            ...
            *line_buf = (char *)kallocate((sizeof(char) * length) + 1, MEMORY_TAG_STRING);
...
KINLINE KAPI b8 filesystem_write_line(file_handle * handle, const char * text){
...
KINLINE KAPI b8 filesystem_read(file_handle * handle, u64 data_size, void * out_data, u64 * out_bytes_read){
...
KINLINE KAPI b8 filesystem_read_all_bytes(file_handle * handle, u8 ** out_bytes,  u64 * out_bytes_read){
        ...
        *out_bytes = (u8 *)kallocate(sizeof(u8) * size, MEMORY_TAG_STRING);
...
KINLINE KAPI b8 filesystem_write(file_handle * handle, u64 data_size, const void * data,  u64 * out_bytes_written){
